 ...
 // view
    ...
    //маленькие кружки и цифры
        ...
        for (var i=1;i<=12;i++){
            ...
            text.style.fontSize=r/8;
            text.textContent=i;     
            var angle=a/180*Math.PI;
        
            var y=r-0.8*r*Math.cos(angle)+r/24;
            if(i<10){
               var x=r+0.8*r*Math.sin(angle)-r/32;  
            }else{
               var x=r+0.8*r*Math.sin(angle)-r/16;
            }
    ...
    //большая -часы
        ...
        strBig.setAttribute("stroke",'black');
    ...
    //средняя -минуты
        ...
        strMdl.setAttribute("stroke",'black');
    ...
    //маленькая -секунды 
        ...
        strSm.setAttribute("stroke",'black');
